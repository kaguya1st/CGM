...
┛┗┛┗┻┗┫┗┻┗┫┗┻  ┗┛┛┗┗┛┗┗┛┛┗┗┗┗┫┗┻  
      ┛   ┛                  ┛    """)
    print(Fore.GREEN + Style.BRIGHT + "Auto Claim Cat Gold Miner")
    print(Fore.BLUE + Style.BRIGHT + "Join TG :  https://t.me/Kaguya_airdrop")
...
def process_quests(authorization):
    ...
    for quest in all_quests:
        ...
        if quest.get('claimStatus', False):
            print(Fore.GREEN + f"Quest sudah diklaim: {quest['questDescription']}. Melewati.")
            continue
...
def process_account(authorization, account_number, total_accounts):
    ...
    print(Fore.WHITE + f"Nama: {user_name}")
    print(Fore.BLUE + f"Lokasi: {assign_location}")
    print(Fore.BLUE + f"Last Login: {last_login_date}")
    ...
    print(Fore.WHITE + f"Total Mata Uang: {total_currency}")
    ...
    if offline_currency['code'] == 0:
        print(Fore.BLUE + f"Mata uang offline: {offline_currency['data']}")
...
def main():
    ...
    print(Fore.BLUE + f"Total akun: {total_accounts}")
    ...
    while True:
        for i, authorization in enumerate(accounts, 1):
            ...
            if i < total_accounts:
                print(Fore.WHITE + "Menunggu 5 detik sebelum memproses akun berikutnya...")
                time.sleep(5)
